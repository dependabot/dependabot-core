    ...
    class UpdateChecker < Dependabot::UpdateCheckers::Base
      ...
      def latest_version
        @latest_version ||= T.let(
          latest_version_finder.latest_release,
      ...
      def lowest_security_fix_version
        @lowest_security_fix_version ||= T.let(
          latest_version_finder.lowest_security_fix_release&.fetch(:version),
      ...
      def updated_ref(source)
        ...
        if vulnerable? && (new_tag = latest_version_finder.lowest_security_fix_release)
        ...
        if source_git_commit_checker.pinned_ref_looks_like_version? &&
           (new_tag = latest_version_finder.latest_version_tag)
          return new_tag.fetch(:tag)
        end
