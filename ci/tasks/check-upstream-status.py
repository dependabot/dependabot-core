...
while num_check < max_checks:
    ...
    if data.status_code != requests.codes.ok:
        ...
        logger.info(
            f"Waiting before checking upstream status again. This was attempt number: {num_check}"
        )
    ...
    response = data.json()
    job_status = [
        (check_run["status"], check_run["conclusion"]) for check_run in response["check_runs"]
    ]
...
for check_run in response["check_runs"]:
    if check_run["status"] != "completed" or check_run["conclusion"] != "success":
        logger.error(
            f"Job name: {check_run['name']} status: {check_run['status']}, conclusion: {check_run['conclusion']}"
        )
