...
module Dependabot
  module Bundler
    class FileParser < Dependabot::FileParsers::Base # rubocop:disable Metrics/ClassLength
      ...
      # This method is used to create a minimally viable requirements array
      # for Dependabot::Dependency objects created from a parsed_lockfile.
      # If the bundep is nil (i.e. the dependency is transitive) then an empty
      # array is returned.
      #
      # By default the list of dependencies generated by this FileParser gets
      # combined into a singular flat list via the DependencySet class. This
      # presents a problem: the Dependency Submission API expects its payload
      # to be a complete representation of the contents of the specified manifest,
      # but once the list has been de-duped we can no longer reliably associate
      # a Dependabot::Dependency with its originating file.
      #
      # Therefore in order to create a DependencySubmission payload we must
      # find a way to associate Dependabot::Dependency objects with their
      # originating DependencyFile.
      #
      # A Dependabot::Dependency object generated from a lockfile needs to track:
      # 1. Whether it has a direct or indirect/transitive relationship
      # 2. Whether it is used in production
      #
      # Dependabot::Dependency#top_level? (aka direct) is true if it has a
      # non-empty `requirements` attribute. So, we need to supply a requirement.
      #
      # Dependabot::Dependency#production? is true (when requirements are present) if
      # the requirement[:groups] includes "runtime".
      sig { params(bundep: T.nilable(::Bundler::Dependency), file_name: String, is_prod: T::Boolean).returns(T::Array[T::Hash[Symbol, T.untyped]]) }
      def bundep_to_requirements(bundep, file_name, is_prod)
        # if bundep is nil then the dependency is transitive, and requirements
        # are expected to be empty
        return [] unless bundep

        # lockfiles do not keep track of groups, so it is impossible for us to
        # accurately fill this out without data from the Gemfile/spec. normally,
        # for transitive deps, this data is supplied through the
        # `subdependency_metadata.production` field using gemfile/spec data supplied
        # by the `production_dep_names` method – so here we use the same field to
        # fake the groups attribute.
        #
        # a future refactor could instead supply the actual requirements array
        # obtained in the equivalent Dependency parsed from the gemfile/gemspec
        if is_prod
          groups = ["runtime"]
        else
          groups = ["development"]
