...
RSpec.describe Dependabot::Bundler::UpdateChecker do
    ...
    context "when a git source" do
      ...
      context "when that is the gem we're checking for" do
        ...
        context "when the gem's tag is pinned" do
          ...
          context "with the gem isn't on Rubygems" do
            before do
              stub_request(:get, rubygems_url + "versions/business.json")
                .to_return(status: 404, body: "This rubygem could not be found.")
            end
          ...
          context "with the reference isn't included in the new version" do
            before do
              allow_any_instance_of(Dependabot::GitCommitChecker)
                .to receive(:branch_or_ref_in_release?)
                .and_return(false)
          ...
          context "with the reference is included in the new version" do
            before do
              allow_any_instance_of(Dependabot::GitCommitChecker)
                .to receive(:branch_or_ref_in_release?)
                .and_return(true)
          ...
          context "with the pin looks like a version" do
            let(:requirements) do
              [{
                file: "Gemfile",
                requirement: ">= 0",
            ...
            context "when but there are no tags" do
  ...
  describe "#latest_resolvable_version" do
    ...
    context "when a gem from rubygems" do
      context "when that only appears in the lockfile" do
        let(:dependency_files) { bundler_project_dependency_files("subdependency") }
        let(:requirements) { [] }
        let(:dependency_name) { "i18n" }
    ...
    context "when a gem with a path source" do
      context "with a downloaded gemspec" do
    ...
    context "when a gem with a git source" do
      let(:dependency_files) { bundler_project_dependency_files("git_source_no_ref") }
      ...
      context "when that is the gem we're checking" do
        ...
        context "when the head of the branch isn't released" do
          ...
          context "with the Gemfile doesn't specify a git source" do
        ...
        context "when the gem's tag is pinned" do
          ...
          context "with the release looks like a version" do
            let(:requirements) do
              [{
                file: "Gemfile",
                requirement: ">= 0",
            ...
            context "when but this dependency has never been released" do
      ...
      context "when that is not the gem we're checking" do
        ...
        context "when that has a bad reference" do
        ...
        context "when that has a bad branch" do
          let(:dependency_files) { bundler_project_dependency_files("bad_branch") }
    ...
    context "when a Gemfile that specifies a Ruby version" do
    ...
    context "with only a Gemfile" do
      ...
      context "when a gem with a private git source" do
      ...
      context "when a gem with a private github source" do
  ...
  describe "#updated_requirements" do
    ...
    context "with a Gemfile and a Gemfile.lock" do
      ...
      context "with a gem with a git source" do
        let(:dependency_files) { bundler_project_dependency_files("git_source_with_version_gemfile") }
        let(:dependency_name) { "dependabot-test-ruby-package" }
        let(:current_version) { "81073f9462f228c6894e3e384d0718def310d99f" }
        let(:requirements) do
        ...
        context "when that is pinned" do
          let(:dependency_files) { bundler_project_dependency_files("git_source") }
  ...
  describe "#requirements_unlocked_or_can_be?" do
    ...
    context "with a Gemfile dependency that is already unlocked" do
      ...
      context "with with the lockfile-only requirements update strategy set" do
    ...
    context "with a Gemfile dependency that can be unlocked" do
      ...
      context "when but with the lockfile-only requirements update strategy set" do
        let(:requirements_update_strategy) { Dependabot::RequirementsUpdateStrategy::LockfileOnly }
