...
RSpec.describe Dependabot::Bundler::FileUpdater::RubyRequirementSetter do
  ...
  describe "#rewrite" do
    ...
    context "when the gemspec includes a required_ruby_version" do
      ...
      context "with a required ruby version multiple args" do
        let(:gemspec) do
          bundler_project_dependency_file("gemspec_required_ruby_version_multiple_args", filename: "example.gemspec")
        end
        let(:content) do
          bundler_project_dependency_file("gemspec_required_ruby_version_multiple_args", filename: "Gemfile").content
        end
      ...
      context "when none of our Ruby versions satisfy" do
        let(:content) do
          bundler_project_dependency_file("gemfile", filename: "Gemfile").content
        end
        let(:gemspec) do
      ...
      context "when requiring ruby 3.4" do
        ...
        it { is_expected.to include("ruby '3.4.7'\n") }
        it { is_expected.to include(%(gem "business", "~> 1.4.0")) }
      end
