...
RSpec.describe Dependabot::Bundler::FileParser do
  ...
  # TODO(brrygrdn): Expand the cases we test for Bundler
  #
  # Bundler graph support is purely opportunistic at this stage in our experiment, in order to make it
  # production-ready, we should expand this example set to cover gemspecs and look into supporting
  # graphs on Gemfile-only projects to align with legacy graphing parsers.
  #
  describe "parse support for DependencySubmission" do
    subject(:dependencies) { parser.parse }

    context "when using a Gemfile and Gemfile.lock" do
      let(:dependency_files) { bundler_project_dependency_files("subdependency") }

      it "attaches a list of direct dependencies to the Gemfile" do
        expect(parser.dependency_files.count).to eq(2)
        gemfile = parser.dependency_files.find { |f| f.name == "Gemfile" }

        direct_dependencies = dependencies.select(&:direct?)

        # assert that direct dependencies are attributed to this file
        expect(gemfile.dependencies).to eq(direct_dependencies.to_set)

        # assert the Gemfile has default priority
        expect(gemfile.priority).to be_zero
      end

      it "attaches a full list of direct and indirect dependencies to the Gemfile.lock" do
        expect(parser.dependency_files.count).to eq(2)
        lockfile = parser.dependency_files.find { |f| f.name == "Gemfile.lock" }
        ...
        # direct dependencies in the global list will be from the Gemfile,
        # so here we check that the Gemfile.lock contains the equivalent
        # by name and version.
        direct.each do |dep|
        ...
        # the Gemfile.lock is the source
        indirect.each do |dependency|
          expect(lockfile.dependencies).to include(dependency)
        end

        # assert the Gemfile.lock is set to a higher priority
