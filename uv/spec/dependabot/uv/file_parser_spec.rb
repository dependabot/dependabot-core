...
RSpec.describe Dependabot::Uv::FileParser do
  ...
  describe "parse" do
    ...
    context "with UV sources path dependencies" do
      ...
      # Mock the Python helper to return UV sources as path dependencies
      before do
        allow_any_instance_of(Dependabot::SharedHelpers)
      ...
      describe "regular dependencies" do
      ...
      describe "UV sources path dependencies" do
        subject(:path_dependency) { dependencies.find { |d| d.name == "protos" } }

        it "parses UV sources path dependencies" do
          expect(path_dependency).to be_a(Dependabot::Dependency)
          expect(path_dependency.name).to eq("protos")
          expect(path_dependency.version).to be_nil
          expect(path_dependency.requirements).to eq([{
            requirement: nil,
            file: "pyproject.toml",
            source: nil,
            groups: ["dependencies"]
      ...
      describe "editable UV sources path dependencies" do
        subject(:editable_dependency) { dependencies.find { |d| d.name == "another-local" } }

        it "parses editable UV sources path dependencies" do
          expect(editable_dependency).to be_a(Dependabot::Dependency)
          expect(editable_dependency.name).to eq("another-local")
          expect(editable_dependency.version).to be_nil
          expect(editable_dependency.requirements).to eq([{
            requirement: nil,
            file: "pyproject.toml",
            source: nil,
            groups: ["dependencies"]
