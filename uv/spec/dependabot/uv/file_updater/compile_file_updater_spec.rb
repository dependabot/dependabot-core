...
RSpec.describe Dependabot::Uv::FileUpdater::CompileFileUpdater do
  ...
  describe "#updated_dependency_files" do
    ...
    context "with cooldown options" do
      ...
      let(:captured_constraints) { [] }

      before do
        allow(updater).to receive(:run_uv_compile_command) do |command, **|
          captured_commands << command
          captured_constraints.concat(
            File.readlines(described_class::COOLDOWN_CONSTRAINTS_FILE, chomp: true)
          )
          File.write(
            generated_file.name,
            generated_file.content.sub("attrs==17.3.0", "attrs==18.1.0")
          )
        end
      end

      it "constrains existing non-target dependencies" do
        expect(updated_files.first.content).to include("attrs==18.1.0")
        expect(captured_commands).to all(include("--constraint\\=.dependabot-cooldown-constraints.txt"))
        expect(captured_constraints).to include("pbr==4.0.2", "pytest==3.5.1")
        expect(captured_constraints).not_to include("attrs==17.3.0")
      end

      context "when compilation changes a non-target dependency" do
        before do
          allow(updater).to receive(:run_uv_compile_command) do
            File.write(
              generated_file.name,
              generated_file.content
                            .sub("attrs==17.3.0", "attrs==18.1.0")
                            .sub("pbr==4.0.2", "pbr==4.0.3")
            )
          end
        end
        ...
        it "rejects the update" do
          expect { updated_files }
            .to raise_error(Dependabot::DependencyFileNotResolvable, /pbr/)
        end
      end

      context "when compilation removes a non-target dependency" do
        before do
          allow(updater).to receive(:run_uv_compile_command) do
            File.write(
              generated_file.name,
        ...
        it "allows the update" do
          expect(updated_files.first.content).to include("attrs==18.1.0")
          expect(updated_files.first.content).not_to include("pbr==")
        end
      end
    end
