...
RSpec.describe Dependabot::Uv::FileUpdater::LockFileUpdater do
  ...
  describe "#required_uv_version" do
    subject(:required_uv_version) { updater.send(:required_uv_version) }

    context "when pyproject.toml contains required-version" do
      let(:pyproject_content) { fixture("pyproject_files", "uv_required_version.toml") }

      it "returns the required version without prefix" do
        expect(required_uv_version).to eq("0.8.22")
      end
    end

    context "when pyproject.toml has required-version with == prefix" do
      let(:pyproject_content) do
        <<~TOML
          [project]
          name = "test"
      ...
      it "strips the == prefix" do
        expect(required_uv_version).to eq("0.9.0")
      end
    end

    context "when pyproject.toml does not contain required-version" do
      let(:pyproject_content) { fixture("pyproject_files", "uv_simple.toml") }

      it "returns nil" do
        expect(required_uv_version).to be_nil
      end
    end

    context "when pyproject.toml is invalid TOML" do
      let(:pyproject_content) { "invalid [ toml" }

      it "returns nil and logs a warning" do
        expect(Dependabot.logger).to receive(:warn).with(/Failed to parse pyproject.toml/)
        expect(required_uv_version).to be_nil
      end
    end

    context "when pyproject file is not present" do
  ...
  describe "#current_uv_version" do
    subject(:current_uv_version) { updater.send(:current_uv_version) }
    ...
    context "when uv is installed" do
      ...
      it "returns the version number" do
        expect(current_uv_version).to eq("0.9.11")
      end
    end

    context "when uv version includes build info" do
      before do
        allow(Dependabot::SharedHelpers).to receive(:run_shell_command)
          .with("pyenv exec uv --version")
          .and_return("uv 0.9.11 (abc123def)")
      end

      it "returns only the version number" do
        expect(current_uv_version).to eq("0.9.11")
      end
    end
    ...
    context "when uv is not installed" do
      before do
        allow(Dependabot::SharedHelpers).to receive(:run_shell_command)
          .with("pyenv exec uv --version")
          .and_raise(StandardError.new("command not found"))
      end

      it "returns nil and logs a warning" do
        expect(Dependabot.logger).to receive(:warn).with(/Failed to get current uv version/)
        expect(current_uv_version).to be_nil
      end
    end
  end

  describe "#ensure_uv_version" do
    subject(:ensure_uv_version) { updater.send(:ensure_uv_version) }
