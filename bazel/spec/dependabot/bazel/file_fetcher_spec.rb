...
RSpec.describe Dependabot::Bazel::FileFetcher do
  ...
  describe "#referenced_files_from_modules" do
    ...
    context "with MODULE.bazel containing from_file references and missing lock files" do
      before do
        stub_request(:get, url + "?ref=sha")
          .to_return(
            status: 200,
            body: fixture("github", "contents_bazel_with_from_file.json"),
            headers: { "content-type" => "application/json" }
          )
        ...
        stub_request(:get, url + "MODULE.bazel?ref=sha")
          .to_return(
            status: 200,
            body: fixture("github", "contents_bazel_module_with_from_file.json"),
            headers: { "content-type" => "application/json" }
          )

        # Mock go.mod file (referenced via from_file)
        stub_request(:get, url + "go.mod?ref=sha")
          .to_return(
            status: 200,
            body: fixture("github", "contents_bazel_go_mod.json"),
            headers: { "content-type" => "application/json" }
          )

        # Mock lock file that exists
        stub_request(:get, url + "containers/distroless/base.lock.json?ref=sha")
          .to_return(
            status: 200,
            body: fixture("github", "contents_bazel_distroless_lock.json"),
            headers: { "content-type" => "application/json" }
          )

        # Mock lock file that doesn't exist yet (will be generated by Bazel)
        stub_request(:get, url + "containers/distroless/util-container.lock.json?ref=sha")
          .to_return(status: 404)

        # Mock directory listings
        stub_request(:get, url + "containers/distroless?ref=sha")
          .to_return(
            status: 200,
            body: '[{"name": "base.lock.json", "type": "file"}, {"name": "BUILD", "type": "file"}]',
            headers: { "content-type" => "application/json" }
          )
        ...
        stub_request(:get, url + "containers/distroless/BUILD?ref=sha")
          .to_return(
            status: 200,
            body: fixture("github", "contents_bazel_distroless_build.json"),
            headers: { "content-type" => "application/json" }
          )

        stub_request(:get, url + "containers/distroless/BUILD.bazel?ref=sha")
          .to_return(status: 404)
      ...
      it "fetches files referenced in from_file attributes" do
        expect(fetched_files.map(&:name)).to include("go.mod")
      end

      it "fetches BUILD files for directories even when referenced file doesn't exist" do
        # containers/distroless/util-container.lock.json doesn't exist, but we still need the BUILD file
        expect(fetched_files.map(&:name)).to include("containers/distroless/BUILD")
      end

      it "fetches files that exist" do
        expect(fetched_files.map(&:name)).to include("containers/distroless/base.lock.json")
      end

      it "doesn't fail when referenced lock file doesn't exist" do
        # util-container.lock.json doesn't exist yet (will be generated by Bazel)
        expect(fetched_files.map(&:name)).not_to include("containers/distroless/util-container.lock.json")
      end
    end
