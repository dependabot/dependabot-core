    ...
    class FileUpdater < Dependabot::FileUpdaters::Base
      ...
      def try_update_projects
        ...
        project_files.each do |project_file|
          ...
          call_nuget_updater_tool(proj_path: proj_path) unless checked_keys.all? do |checked_key|
            checked_files.include?(checked_key)
          end
          ...
          downstream_files.each do |downstream_file|
            @dependencies.each { |dep| checked_files.add("#{downstream_file}-#{dep.name}#{dep.version}") }
          end
          update_ran = true
        end
      ...
      def try_update_json
        if contains_updated_dependency(dotnet_tools_json_dependencies) ||
          ...
          call_nuget_updater_tool(proj_path: proj_path)
          return true
        end
