...
public class RunWorker
{
    ...
    public async Task RunAsync(FileInfo jobFilePath, DirectoryInfo repoContentsPath, string baseCommitSha, FileInfo outputFilePath)
    {
        ...
        if (!experimentsManager.UseGroupRunner)
        {
            // the group runner doesn't write this file
    ...
    public async Task<RunResult> RunAsync(Job job, DirectoryInfo repoContentsPath, string baseCommitSha, ExperimentsManager experimentsManager)
    {
        ...
        if (experimentsManager.UseGroupRunner)
        {
            await RunGroupsWithErrorHandlingAsync(job, repoContentsPath, baseCommitSha, experimentsManager);
    ...
    private async Task RunGroupsWithErrorHandlingAsync(Job job, DirectoryInfo repoContentsPath, string baseCommitSha, ExperimentsManager experimentsManager)
    {
