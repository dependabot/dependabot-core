        ...
        ILogger logger)
    {
        ...
        foreach (var tfm in tfms)
        {
            ...
            foreach (var dependency in dependencies)
            {
                ...
                var names = new[] { packageName };
                if (dependencyName.Contains(";"))
                {
                    names = dependencyName.Split(';', StringSplitOptions.RemoveEmptyEntries);
                }

                if (packageName.Equals(dependencyName, StringComparison.OrdinalIgnoreCase) || names.Contains(packageName))
                {
                    packageFound = true;
