        ...
        foreach (var discoveredTfm in discoveredTfms)
        {
            ...
            try
            {
                ...
                if (experimentsManager.UseSingleRestore)
                {
                    ...
                    {
                        // call a `Build` target from the meta project that executes `InnerTargets`
                        var extension = Path.GetExtension(startingProjectPath)?.ToLowerInvariant();
                        if (extension == ".sln" || extension == ".slnx")
                        {
                            // solution files don't specify any targets directly, but a pseudo `Restore` target is available and and required for `Build`
                            args.Add("/t:Restore,Build");
                        }
                        else
                        {
                            args.Add("/t:Build");
