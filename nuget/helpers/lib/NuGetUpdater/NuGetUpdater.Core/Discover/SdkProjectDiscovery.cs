...
internal static class SdkProjectDiscovery
{
    ...
    public static async Task<ImmutableArray<ProjectDiscoveryResult>> DiscoverAsync(string repoRootPath, string workspacePath, string startingProjectPath, Logger logger)
    {
        ...
        // the following collectsion feature heavily; the shape is described as follows
        ...
        // and done
        var projectDiscoveryResults = packagesPerProject.Keys.Select(projectPath =>
        {
            ...
            {
                return packagesByTfm[tfm].Keys.Select(packageName =>
                {
            ...
            }).OrderBy(d => d.Name).ToImmutableArray();
