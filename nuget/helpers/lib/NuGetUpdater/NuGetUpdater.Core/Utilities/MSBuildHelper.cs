...
internal static partial class MSBuildHelper
{
    ...
    public static async Task<T> SidelineGlobalJsonAsync<T>(string currentDirectory, string rootDirectory, Func<Task<T>> action, bool retainMSBuildSdks = false)
    {
    ...
    internal static async Task<ImmutableArray<string>> GetTargetFrameworkValuesFromProject(string repoRoot, string projectPath, Logger logger)
    {
