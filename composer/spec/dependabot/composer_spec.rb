...
RSpec.describe Dependabot::Composer do
  ...
  it "has the same binary version of composer installed as specified in the native helper" do
    ...
    #
    # If you've updated the composer version in composer/helpers/composer.lock,
    # you also need to bump it in the Dockerfile.

    expect(helper_composer_version).to eq(native_composer_version)
  end

  private

  def helper_composer_version
    composer_lock = File.read(File.join(Dependabot::Composer::NativeHelpers.composer_helpers_dir, "composer.lock"))
