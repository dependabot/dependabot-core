...
RSpec.describe Dependabot::Composer::FileUpdater::LockfileUpdater do
  ...
  describe "the updated lockfile" do
    ...
    context "when that requires an environment variable (composer v1)" do
      let(:project_name) { "v1/env_variable" }

      context "when that hasn't been provided" do
      ...
      context "when that has been provided" do
        let(:updater) do
          described_class.new(
            dependency_files: files,
            dependencies: [dependency],
    ...
    context "when that requires an environment variable (composer v2)" do
    ...
    context "when the dependency is a subdependency" do
      ...
      context "with is limited by a library's PHP version" do
    ...
    context "with regression spec for media-organizer" do
