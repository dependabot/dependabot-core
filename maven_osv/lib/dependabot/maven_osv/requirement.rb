...
module Dependabot
  module MavenOSV
    class Requirement < Dependabot::Requirement
      ...
      OR_SYNTAX = /(?<=\]|\)),/
      PATTERN_RAW = "\\s*(#{quoted})?\\s*(#{MavenOSV::Version::VERSION_PATTERN})\\s*".freeze
      PATTERN = /\A#{PATTERN_RAW}\z/

      def self.parse(obj)
        return ["=", MavenOSV::Version.new(obj.to_s)] if obj.is_a?(Gem::Version)
        ...
        [matches[1] || "=", MavenOSV::Version.new(T.must(matches[2]))]
      end
      ...
      def satisfied_by?(version)
        version = MavenOSV::Version.new(version.to_s)
        super
      end
...
Dependabot::Utils
  .register_requirement_class("maven_osv", Dependabot::MavenOSV::Requirement)
