      ...
      class DeclarationFinder
        ...
        def declaring_requirement_matches?(node)
          ...
          if declaring_requirement.dig(:metadata, :property_name)
            ...
            property_name =
              node_requirement
              .match(MavenOSV::FileParser::PROPERTY_REGEX)
              &.named_captures
              &.fetch("property")
        ...
        def evaluated_value(value)
          return value unless value.match?(MavenOSV::FileParser::PROPERTY_REGEX)

          property_name =
            value.match(MavenOSV::FileParser::PROPERTY_REGEX)
          ...
          value.gsub(
            value.match(MavenOSV::FileParser::PROPERTY_REGEX).to_s,
            property_value
          )
        end
