    ...
    module Operations
      class UpdateAllVersions
        ...
        def existing_pull_request(updated_dependencies)
          return unless updated_dependencies.any?

          new_pr_set_with_dir = create_pr_set(updated_dependencies, include_directory: true)
          existing_pull_request = find_existing_pull_request(new_pr_set_with_dir)

          return existing_pull_request if existing_pull_request

          # Try again without directory in case the data is old
          new_pr_set_without_dir = create_pr_set(updated_dependencies, include_directory: false)
          find_existing_pull_request(new_pr_set_without_dir)
        end

        sig do
          params(updated_dependencies: T::Array[Dependabot::Dependency], include_directory: T::Boolean)
            .returns(T::Set[T::Hash[String, T.untyped]])
        end
        def create_pr_set(updated_dependencies, include_directory:)
          updated_dependencies.to_set do |dep|
            pr_data = {
              "dependency-name" => dep.name,
              "dependency-version" => dep.version,
              "dependency-removed" => dep.removed? ? true : nil
            }
            pr_data["directory"] = job.source.directory if include_directory
            pr_data.compact
          end.to_set
        end

        sig do
          params(pr_set: T::Set[T::Hash[String, T.untyped]])
            .returns(T.nilable(T::Array[T::Hash[String, T.untyped]]))
        end
        def find_existing_pull_request(pr_set)
          job.existing_pull_requests.find { |pr| Set.new(pr) == pr_set } ||
            created_pull_requests.find { |pr| Set.new(pr) == pr_set }
        end
          ...
          updated_deps
            ...
            .any? do |dep|
              ...
              original_peer_dep = ::Dependabot::Dependency.new(
                ...
                requirements: dep_previous_requirements,
