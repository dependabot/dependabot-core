...
RSpec.describe Dependabot::FileFetcherCommand do
  ...
  describe "#glob?" do
    ...
    it "returns true for directories containing asterisk" do
      expect(command.send(:glob?, "/path/*/dir")).to be true
    end

    it "returns true for directories containing question mark" do
      expect(command.send(:glob?, "/path/?/dir")).to be true
    end

    it "returns true for directories containing square brackets" do
      expect(command.send(:glob?, "/path/[abc]/dir")).to be true
    end

    it "returns true for directories containing curly braces" do
      expect(command.send(:glob?, "/path/{a,b}/dir")).to be true
      expect(command.send(:glob?, "{{ dir.with.special.chars }}")).to be true
    end

    it "returns false for directories with only opening square bracket" do
      expect(command.send(:glob?, "/path/[incomplete")).to be false
    end

    it "returns false for directories with only closing square bracket" do
      expect(command.send(:glob?, "/path/incomplete]")).to be false
    end

    it "returns false for directories with only opening curly brace" do
      expect(command.send(:glob?, "/path/{incomplete")).to be false
    end

    it "returns false for directories with only closing curly brace" do
      expect(command.send(:glob?, "/path/incomplete}")).to be false
    end

    it "returns false for normal directories" do
      expect(command.send(:glob?, "/path/to/dir")).to be false
    end

    it "returns false for directories with special characters that are not glob patterns" do
      expect(command.send(:glob?, "/path/with-dashes/and_underscores")).to be false
      expect(command.send(:glob?, "/path with spaces")).to be false
    end
  end
end
