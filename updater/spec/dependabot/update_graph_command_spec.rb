...
RSpec.describe Dependabot::UpdateGraphCommand do
  ...
  describe "#perform_job when there is an error parsing the dependency files" do
    ...
    before do
      ...
      mock_parser_class = double(Dependabot::FileParsers::Base)
      allow(mock_parser_class).to receive(:new) { mock_parser }
      allow(Dependabot::FileParsers).to receive(:for_package_manager) { mock_parser_class }
    end
