...
RSpec.describe ExceptionSanitizer do
  ...
  it "does not filter messages by default" do
    expect(exception).to have_received(:value=).with(message).at_least(:once)
  end
  ...
  context "with exception containing Bearer token" do
    ...
    it "filters sensitive messages" do
      expect(exception).to have_received(:value=).with("Bearer [FILTERED_AUTH_TOKEN] is bad and you should feel bad")
    end
  end
  ...
  context "with exception containing Authorization: header" do
    ...
    it "filters sensitive messages" do
      expect(exception).to have_received(:value=).with("Authorization: [FILTERED_AUTH_TOKEN] is bad")
    end
  end
  ...
  context "with exception containing authorization value" do
    ...
    it "filters sensitive messages" do
      expect(exception).to have_received(:value=).with("authorization [FILTERED_AUTH_TOKEN] invalid")
    end
  end
  ...
  context "with api repo NWO" do
    ...
    it "filters repo name from an api request" do
      expect(exception).to have_received(:value=).with("https://api.github.com/repos/foo/[FILTERED_REPO] is bad")
    end
  end
  ...
  context "with regular repo NWO" do
    ...
    it "filters repo name from an api request" do
      expect(exception).to have_received(:value=).with("https://github.com/foo/[FILTERED_REPO] is bad")
    end
  end
  ...
  context "with multiple repo NWO" do
    ...
    it "filters repo name from an api request" do
      expect(exception).to have_received(:value=).with(
  ...
  context "when docs.github.com URL included" do
    ...
    it "filters repo name from an api request" do
      expect(exception).to have_received(:value=)
  ...
  context "with SCP-style uri" do
    ...
    it "filters repo name from an api request" do
      expect(exception).to have_received(:value=).with("git@github.com:foo/[FILTERED_REPO] is bad")
    end
  end
end
