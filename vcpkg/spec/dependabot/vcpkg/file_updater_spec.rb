...
RSpec.describe Dependabot::Vcpkg::FileUpdater do
  ...
  describe "#updated_files_regex" do
    ...
    it "matches vcpkg-configuration.json files" do
      expect(updated_files_regex).to include(match("vcpkg-configuration.json"))
      expect(updated_files_regex).to include(match("path/to/vcpkg-configuration.json"))
    end
  end
end
