...
RSpec.describe Dependabot::Helm::Version do
  ...
  describe "#to_s" do
    it "returns the version string for standard versions" do
      expect(described_class.new("2.4.2").to_s).to eq("2.4.2")
    end

    it "returns the version string with digest when present" do
      version_with_digest = "1.0.119807+c2277fddd003556d4982b86ef4e77fc84a41ed79"
      expect(described_class.new(version_with_digest).to_s).to eq(version_with_digest)
    end

    it "returns the version string without update part" do
      expect(described_class.new("11.0.16_8").to_s).to eq("11.0.16")
    end
