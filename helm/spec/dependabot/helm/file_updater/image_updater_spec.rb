...
RSpec.describe Dependabot::Helm::FileUpdater::ImageUpdater do
  ...
  describe "#updated_values_yaml_content" do
    ...
    context "with a default-chomping (clip) block-style scalar image" do
      let(:dependency_name) { "nginx" }
      let(:dependency_version) { "1.21.0" }
      let(:dependency_previous_version) { "1.20.0" }
      let(:dependency_requirements) do
      ...
      it "updates the tag despite Psych including a trailing newline in the parsed value" do
        updated_content = updater.updated_values_yaml_content("values.yaml")
    ...
    context "with an escaped quoted image followed by a sibling with the same literal value" do
      let(:dependency_name) { "nginx" }
      let(:dependency_version) { "1.21.0" }
      let(:dependency_previous_version) { "1.20.0" }
      let(:dependency_requirements) do
      ...
      let(:fixture_content) { "config: {image: \"nginx:1.20.\\x30\", other: nginx:1.20.0}\n" }

      it "raises rather than editing the sibling when the literal isn't in the node's own source" do
        expect { updater.updated_values_yaml_content("values.yaml") }
          .to raise_error("Expected content to change!")
      end
    end
