...
RSpec.describe Dependabot::Javascript::Shared::MetadataFinder do
  extend T::Sig

  subject(:finder) do
    T.let(described_class.new(dependency: dependency, credentials: credentials),
          described_class)
  end

  let(:dependency_name) { T.let("etag", String) }
  let(:credentials) do
    T.let([Dependabot::Credential.new({
      "type" => "git_source",
      "host" => "github.com",
  ...
  let(:dependency) do
    T.let(Dependabot::Dependency.new(
