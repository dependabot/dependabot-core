...
RSpec.describe Dependabot::Javascript::FileUpdater::PackageJsonUpdater do
  ...
  let(:project_name) { "javascript/simple" }
  ...
  describe "#updated_package_json" do
    ...
    context "when the minor version is specified" do
      ...
      let(:project_name) { "javascript/minor_version_specified" }
    ...
    context "when the requirement hasn't changed" do
      ...
      its(:content) do
        is_expected.to eq(fixture("projects", "javascript", "minor_version_specified", "package.json"))
      end
    ...
    context "when updating multiple dependencies that results in 'package.json' update only once" do
      ...
      let(:project_name) { "javascript/simple_with_multiple_deps" }
      let(:dependencies) do
        [
          Dependabot::Dependency.new(
            name: "cross-spawn",
    ...
    context "when the dependency is specified as both dev and runtime" do
      ...
      let(:project_name) { "javascript/duplicate" }
      ...
      context "with identical versions" do
        let(:project_name) { "javascript/duplicate_identical" }
    ...
    context "when the dependency is specified as both dev and peer" do
      ...
      let(:project_name) { "javascript/dev_and_peer_dependency" }
    ...
    context "with a git dependency" do
      let(:project_name) { "javascript/github_dependency" }
      let(:dependency) do
        Dependabot::Dependency.new(
          name: "is-number",
      ...
      context "when it specifies a semver requirement" do
        let(:project_name) { "javascript/github_dependency_semver" }
        let(:dependency) do
          Dependabot::Dependency.new(
            name: "is-number",
            version: "4.0.0",
    ...
    context "with a path-based dependency" do
      let(:project_name) { "javascript/path_dependency" }
      let(:dependency) do
        Dependabot::Dependency.new(
          name: "lodash",
          version: "1.3.1",
    ...
    context "with non-standard whitespace" do
      let(:project_name) { "javascript/non_standard_whitespace" }
