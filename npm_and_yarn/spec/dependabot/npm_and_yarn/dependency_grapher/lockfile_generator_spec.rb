    ...
    context "with npm package manager" do
      ...
      it "attempts to generate a package-lock.json" do
        expect(Dependabot::NpmAndYarn::Helpers).to receive(:run_npm_command)
          .with("install --package-lock-only --ignore-scripts --force",
    ...
    context "with yarn package manager" do
      ...
      context "with Yarn Classic (no .yarnrc.yml)" do
        ...
        it "runs yarn install with appropriate flags" do
          expect(Dependabot::SharedHelpers).to receive(:run_shell_command)
            .with("yarn install --ignore-scripts --frozen-lockfile=false",
      ...
      context "with Yarn Berry (.yarnrc.yml present)" do
        ...
        it "runs yarn install with update-lockfile mode" do
          ...
          expect(Dependabot::NpmAndYarn::Helpers).to receive(:setup_yarn_berry)
          expect(Dependabot::NpmAndYarn::Helpers).to receive(:run_yarn_command)
    ...
    context "with pnpm package manager" do
      ...
      it "runs pnpm install with lockfile-only flag" do
        expect(Dependabot::NpmAndYarn::Helpers).to receive(:run_pnpm_command)
          .with("install --lockfile-only --ignore-scripts",
