...
RSpec.describe Dependabot::NpmAndYarn::FileUpdater::NpmLockfileUpdater do
  ...
  describe "npm 6 specific" do
    ...
    context "with git sub-dependency with invalid from that is updating from an npm5 lockfile" do
  ...
  context "when workspace with outdated deps not in root package.json" do
  ...
  %w(npm6 npm8).each do |npm_version|
    describe "#{npm_version} updates" do
      ...
      context "with git sub-dependency with invalid from" do
