...
RSpec.describe Dependabot::NpmAndYarn::UpdateChecker::RequirementsUpdater do
  ...
  describe "#updated_requirements" do
    ...
    context "with a dist tag" do
      ...
      context "when that starts with a v" do
    ...
    context "with a requirement having its version bumped" do
      let(:update_strategy) { Dependabot::RequirementsUpdateStrategy::BumpVersions }
      ...
      context "when there is a resolvable version" do
        ...
        context "with v-prefix was previously used" do
          let(:package_json_req_string) { "v1.2.3" }
          its([:requirement]) { is_expected.to eq("v1.5.0") }

          context "when that is capitalised (and therefore invalid)" do
            let(:package_json_req_string) { "V1.2.3" }
        ...
        context "with an x.x was previously specified" do
          let(:package_json_req_string) { "^0.x.x" }
          its([:requirement]) { is_expected.to eq("^1.x.x") }
        end

        context "with an x.x was previously specified with four places" do
        ...
        context "with there were multiple requirements" do
          ...
          context "when one of which was a pre-release" do
            let(:package_json_req_string) { "0.4.5" }
            let(:other_requirement_string) { "1.1.0-alpha.1" }

            context "with to a new pre-release version" do
              let(:latest_resolvable_version) do
    ...
    context "with a requirement having its version bumped if required" do
      ...
      context "when there is a resolvable version" do
        ...
        context "with a caret was previously specified" do
          ...
          context "when that this version doesn't satisfy" do
    ...
    context "with a requirement being widened" do
      ...
      context "when there is a resolvable version" do
        ...
        context "with a tilde was previously specified" do
          ...
          context "with including an x" do
        ...
        context "with there were multiple specifications" do
          ...
          context "with specified with || and valid" do
    ...
    context "with a requirement being left alone" do
