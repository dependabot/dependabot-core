...
async function findConflictingDependencies(directory, depName, targetVersion) {
    const lockfileJson = await (0, lockfile_parser_js_1.parse)(directory);
    const packageJson = fs_1.default
        .readFileSync(path_1.default.join(directory, "package.json"))
        .toString();
    ...
    const conflictingParents = topLevelDependencies.flatMap(([topLevelDepName, topLevelRequirement]) => {
        const topLevelSpec = {
            name: topLevelDepName,
            requirement: topLevelRequirement,
        };
        return Array.from(findConflictingParentDependencies(topLevelDepName, topLevelRequirement, depName, targetVersion, topLevelSpec, lockfileJson).values());
    });
...
function buildExplanation(parentSpec, targetDepName) {
    if (parentSpec.name === parentSpec.topLevelSpec.name &&
        parentSpec.version === parentSpec.topLevelSpec.version) {
        // The nodes parent is top-level
        return (`${parentSpec.name}@${parentSpec.version} requires ${targetDepName}` +
            `@${parentSpec.requirement}`);
    }
    else if (parentSpec.transitiveSpec.name === parentSpec.topLevelSpec.name &&
        ...
        return (`${parentSpec.topLevelSpec.name}@${parentSpec.topLevelSpec.version} requires ` +
...
function findConflictingParentDependencies(dependency, requirement, targetDep, targetversion, topLevelSpec, lockfileJson, transitiveSpec = {}, checkedEntries = new Set(), conflictingParents = new Map()) {
    ...
    for (const [entry, pkg] of Object.entries(lockfileJson)) {
        ...
        if (topLevelSpec.name == parentDepName &&
        ...
        if (pkg.dependencies &&
            ...
            for (const [subDepName, spec] of Object.entries(pkg.dependencies)) {
                if (subDepName === targetDep &&
                    !semver_1.default.satisfies(targetversion, spec)) {
                    ...
                    findConflictingParentDependencies(subDepName, spec, targetDep, targetversion, topLevelSpec, lockfileJson, transitiveSpec, checkedEntries, conflictingParents);
                }
            }
        }
    }
