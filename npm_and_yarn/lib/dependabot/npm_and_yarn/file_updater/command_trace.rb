...
module Dependabot
  module NpmAndYarn
    class FileUpdater < Dependabot::FileUpdaters::Base
      # Captures diagnostic data about a single native package-manager
      # command invocation (npm / yarn / pnpm) so that "no change" errors
      # can be debugged with full context: whether each command succeeded,
      # its duration, truncated output, and whether the lockfile content
      # changed as a result.
      #
      # Usage:
      #
      ...
      class CommandTrace
        ...
        # Wraps a native-command invocation, recording timing, success/
        # failure state, and (truncated) output into a new CommandTrace
        # appended to `traces`. Re-raises any exception after recording so
        # callers can keep their existing error-handling flow.
        sig do
