...
module Dependabot
  module NpmAndYarn
    class UpdateChecker
      class LockedSubdependencyVersionResolver
        def initialize(dependency:, credentials:, dependency_files:,
                       ignored_versions:, latest_allowable_version:,
                       locking_dependencies:)
          @dependency = dependency
          @credentials = credentials
          @dependency_files = dependency_files
          @ignored_versions = ignored_versions
          @latest_allowable_version = latest_allowable_version
          @locking_dependencies = locking_dependencies
        end
        ...
        def update_subdependency_in_lockfile(lockfile)
          ...
          updated_lockfile = lockfile.dup
          updated_lockfile.name = lockfile_name
          updated_lockfile.directory = path

          updated_files = if lockfile.name.end_with?("yarn.lock")
                            run_yarn_updater(path, lockfile_name)
                          else
                            run_npm_updater(path, updated_lockfile)
                          end
        ...
        def run_npm_updater(path, lockfile)
          SharedHelpers.with_git_configured(credentials: credentials) do
            Dir.chdir(path) do
              npm_version = Dependabot::NpmAndYarn::Helpers.npm_version(lockfile.content)

              updater = FileUpdater::NpmLockfileUpdater.new(
                lockfile: lockfile,
                dependencies: [dependency] + @locking_dependencies,
                dependency_files: dependency_files,
                credentials: @credentials
              )
              { lockfile.name => updater.updated_lockfile.content }
              # if npm_version == "npm8"
