...
RSpec.describe Dependabot::Swift::Version do
  ...
  describe ".new" do
    ...
    it "keeps a malformed build tail distinct and sorts it below the release" do
      # Only well-formed build metadata is stripped; a malformed "+…" tail (tolerated at
      # construction, though .correct? rejects it) must not canonicalize to the bare release.
      ["1.0.0++", "1.0.0+", "1.0.0+."].each do |tag|
        malformed = described_class.new(tag)
        release = described_class.new("1.0.0")

        expect(malformed.to_semver).to eq(tag)
        expect(malformed).not_to eq(release)
        expect(malformed).not_to eql(release)
        expect(malformed).to be < release
      end
    end
    ...
    it "treats an empty explicit suffix as a pre-release that sorts below the release" do
      # "1.0.0-" must sort below the stable "1.0.0", not masquerade as it.
      version = described_class.new("1.0.0-")
      expect(version.prerelease?).to be true
      expect(version).to be < described_class.new("1.0.0")
    end

    it "keeps a non-numeric core intact instead of splitting on an internal hyphen" do
      # "1.0.alpha-beta" has a non-numeric core, so the whole tail is the pre-release, not just "beta".
      expect(described_class.new("1.0.alpha-beta")).not_to eq(described_class.new("1.0-beta"))
      expect(described_class.new("1.0.alpha-beta")).not_to eq(described_class.new("1.0.gamma-beta"))
    end
  ...
  describe "comparison" do
    ...
    context "with SemVer section 11 pre-release precedence" do
      ...
      it "keeps SemVer-invalid leading-zero identifiers distinct and ordered deterministically" do
        padded = described_class.new("1.0.0-01")
        plain = described_class.new("1.0.0-1")

        expect(padded).not_to eq(plain)
        expect(padded).to be < plain
      end
    end
  end

  describe "generated ignore ranges" do
    subject(:version) { described_class.new("1.2.3") }

    it "emits a strict-SemVer minor range the Swift requirement grammar accepts" do
      expect(version.ignored_minor_versions).to eq([">= 1.3.0-0, < 2.0.0-0"])
      expect { Dependabot::Swift::Requirement.new(version.ignored_minor_versions.first) }.not_to raise_error
    end

    it "emits a strict-SemVer major floor the Swift requirement grammar accepts" do
      expect(version.ignored_major_versions).to eq([">= 2.0.0-0"])
      expect { Dependabot::Swift::Requirement.new(version.ignored_major_versions.first) }.not_to raise_error
    end

    it "keeps the minor range boundaries correct, including prereleases" do
      requirement = Dependabot::Swift::Requirement.new(version.ignored_minor_versions.first)

      expect(requirement.satisfied_by?(described_class.new("1.2.9"))).to be false
      expect(requirement.satisfied_by?(described_class.new("1.3.0-0"))).to be true
      expect(requirement.satisfied_by?(described_class.new("1.3.0-alpha"))).to be true
      expect(requirement.satisfied_by?(described_class.new("1.9.9"))).to be true
      # A next-major prerelease must stay outside the minor range.
      expect(requirement.satisfied_by?(described_class.new("2.0.0-alpha"))).to be false
      expect(requirement.satisfied_by?(described_class.new("2.0.0"))).to be false
    end

    it "covers numeric next-major prereleases in the major floor" do
      requirement = Dependabot::Swift::Requirement.new(version.ignored_major_versions.first)

      expect(requirement.satisfied_by?(described_class.new("1.9.9"))).to be false
      expect(requirement.satisfied_by?(described_class.new("2.0.0-0"))).to be true
      expect(requirement.satisfied_by?(described_class.new("2.0.0-alpha"))).to be true
      expect(requirement.satisfied_by?(described_class.new("2.0.0"))).to be true
    end
  end
end
