...
RSpec.describe Dependabot::Clients::CodeCommit do
  ...
  describe "#truncate_pr_description" do
    subject { described_class.new.truncate_pr_description(pr_description) }

    let(:pr_description) { "This is a normal length PR description and it should not be truncated." }

    context "when the pull request description is within the allowed length" do
      it "returns the original description" do
        expect(subject).to eq(pr_description)
      end
    end

    context "when the pull request description exceeds the allowed length" do
    ...
    context "when the pull request description is already truncated" do
      let(:pr_description) { "This is a truncated description...\n\n_Description has been truncated_" }

      it "returns the original truncated description" do
        expect(subject).to eq(pr_description)
      end
    end
