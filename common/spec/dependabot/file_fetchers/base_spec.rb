  ...
  describe "#commit" do
    ...
    context "with a GitHub source" do
      ...
      context "with a target branch" do
        ...
        context "when that returns an array (because it is a substring)" do
          before do
            stub_request(:get, url + "/git/refs/heads/my_branch")
              .with(headers: { "Authorization" => "token token" })
              .to_return(status: 200,
  ...
  describe "#files" do
    ...
    context "with a GitHub source" do
      ...
      context "with a directory specified" do
        context "when that ends in a slash" do
          let(:directory) { "app/" }
        ...
        context "when that begins with a slash" do
          let(:directory) { "/app" }
          let(:url) { "https://api.github.com/repos/#{repo}/contents/app/" }
        ...
        context "when that includes a slash" do
          let(:directory) { "a/pp" }
      ...
      context "when the file is in a directory" do
        ...
        context "when that is in a submodule (shallow)" do
          before do
            stub_request(:get, url + "some/dir/req.txt?ref=sha")
              .with(headers: { "Authorization" => "token token" })
              .to_return(status: 404)
        ...
        context "when that is in a submodule (deep)" do
          before do
            stub_request(:get, url + "some/dir/req.txt?ref=sha")
              .with(headers: { "Authorization" => "token token" })
              .to_return(status: 404)
        ...
        context "when that is in a symlinked directory" do
          before do
            stub_request(:get, url + "some/dir/req.txt?ref=sha")
              .with(headers: { "Authorization" => "token token" })
              .to_return(status: 404)
    ...
    context "with a CodeCommit source" do
      ...
      context "with a directory specified" do
        ...
        context "when that beings with a slash" do
          before do
            stubbed_cc_client
              .stub_responses(
                :get_file,
