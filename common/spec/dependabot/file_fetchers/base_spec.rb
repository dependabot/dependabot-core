...
RSpec.describe Dependabot::FileFetchers::Base do
  ...
  describe "filename normalization in DependencyFile creation" do
    # Unit tests to verify filename normalization logic
    describe "filename path normalization" do
      require "pathname"
      ...
      it "normalizes various filename formats correctly" do
        test_cases = [
          ...
          ["/./subdir/../Cargo.toml", "Cargo.toml"], # Leading slash is stripped, then path normalized
    ...
    describe "file parser compatibility" do
      ...
      it "allows file parser to find files with normalized names" do
        # Create a file with problematic absolute path name (the old bug)
        problematic_file = Dependabot::DependencyFile.new(
          name: "/Cargo.toml", # This would cause "No Cargo.toml!" error
          content: "[package]\nname = \"test\"\n"
        )

        # Verify the problem exists with non-normalized names
        expect { cargo_parser_class.new(dependency_files: [problematic_file]) }
          .to raise_error("No Cargo.toml!")

        # Create a file with properly normalized name (our fix)
        normalized_file = Dependabot::DependencyFile.new(
          name: "Cargo.toml", # Properly normalized name
          content: "[package]\nname = \"test\"\n"
        )

        # Verify the fix works
        expect { cargo_parser_class.new(dependency_files: [normalized_file]) }
          .not_to raise_error
      end
    end
  end
