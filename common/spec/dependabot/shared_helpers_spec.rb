...
RSpec.describe Dependabot::SharedHelpers do
  ...
  describe ".excon_connect_timeout_in_seconds" do
    context "when DEPENDABOT_EXCON_CONNECT_TIMEOUT_IN_SECONDS is set" do
      it "returns the provided value" do
        override_value = 10
        stub_const("ENV", ENV.to_hash.merge("DEPENDABOT_EXCON_CONNECT_TIMEOUT_IN_SECONDS" => override_value.to_s))
        expect(described_class.excon_connect_timeout_in_seconds).to eq(override_value)
      end
    end

    context "when ENV does not provide an override" do
      it "falls back to a default value" do
        expect(described_class.excon_connect_timeout_in_seconds)
          .to eq(Dependabot::SharedHelpers::DEFAULT_EXCON_CONNECT_TIMEOUT_IN_SECONDS)
      end
    end
  end

  describe ".excon_write_timeout_in_seconds" do
    context "when DEPENDABOT_EXCON_WRITE_TIMEOUT_IN_SECONDS is set" do
      it "returns the provided value" do
        override_value = 15
        stub_const("ENV", ENV.to_hash.merge("DEPENDABOT_EXCON_WRITE_TIMEOUT_IN_SECONDS" => override_value.to_s))
        expect(described_class.excon_write_timeout_in_seconds).to eq(override_value)
      end
    end

    context "when ENV does not provide an override" do
      it "falls back to a default value" do
        expect(described_class.excon_write_timeout_in_seconds)
          .to eq(Dependabot::SharedHelpers::DEFAULT_EXCON_WRITE_TIMEOUT_IN_SECONDS)
      end
    end
  end

  describe ".excon_read_timeout_in_seconds" do
    context "when DEPENDABOT_EXCON_READ_TIMEOUT_IN_SECONDS is set" do
      it "returns the provided value" do
        override_value = 30
        stub_const("ENV", ENV.to_hash.merge("DEPENDABOT_EXCON_READ_TIMEOUT_IN_SECONDS" => override_value.to_s))
        expect(described_class.excon_read_timeout_in_seconds).to eq(override_value)
      end
    end

    context "when ENV does not provide an override" do
      it "falls back to a default value" do
        expect(described_class.excon_read_timeout_in_seconds)
          .to eq(Dependabot::SharedHelpers::DEFAULT_EXCON_READ_TIMEOUT_IN_SECONDS)
      end
    end
  end

  describe ".excon_retry_limit" do
    context "when DEPENDABOT_EXCON_RETRY_LIMIT is set" do
      it "returns the provided value" do
        override_value = 10
        stub_const("ENV", ENV.to_hash.merge("DEPENDABOT_EXCON_RETRY_LIMIT" => override_value.to_s))
        expect(described_class.excon_retry_limit).to eq(override_value)
      end
    end

    context "when ENV does not provide an override" do
      it "falls back to a default value" do
        expect(described_class.excon_retry_limit)
          .to eq(Dependabot::SharedHelpers::DEFAULT_EXCON_RETRY_LIMIT)
      end
    end
  end
