...
RSpec.describe Dependabot::SharedHelpers do
  ...
  describe ".with_git_configured" do
    ...
    let(:git_store_dir) { File.join(Dir.tmpdir, "dependabot-git-store-spec") }
    let(:git_store_path) { File.join(git_store_dir, "XXXXXXXXXXXXXXXX.git.store") }
    ...
    let(:credential_helper_command) do
      "git config --global credential.helper " \
        "'!#{described_class.credential_helper_path} --file #{git_store_path}'"
    end

    let(:credentials_helper) do
      "[credential]\n\thelper = !#{described_class.credential_helper_path} --file #{git_store_path}"
    end
    ...
    before do
      ...
      allow(Dir).to receive(:mktmpdir).with("dependabot_git_store").and_return(git_store_dir)
      FileUtils.mkdir_p(git_store_dir)
    end

    after do
      FileUtils.rm_rf(git_store_dir)
    end
    ...
    context "when providing no extra credentials" do
      ...
      it "creates the git credentials store outside the working directory" do
        with_git_configured do
          expect(git_store_path).to start_with(git_store_dir)
          expect(git_store_path).not_to start_with(Dir.pwd)
        end
      end

      it "creates the git credentials store with private permissions" do
        with_git_configured do
          expect(File.stat(git_store_path).mode & 0o777).to eq(0o600)
        end
      end

      it "removes the git credentials store after use" do
        with_git_configured { expect(File.exist?(git_store_path)).to be(true) }

        expect(File.exist?(git_store_path)).to be(false)
        expect(Dir.exist?(git_store_dir)).to be(false)
      end
    end
    ...
    context "when a git helper command fails after creating the credentials store" do
      let(:helper_failure) do
        described_class::HelperSubprocessFailed.new(
          message: "git config failed",
          error_context: { command: credential_helper_command }
        )
      end

      before do
        allow(described_class).to receive(:run_shell_command).and_call_original
        allow(described_class)
          ...
          .and_raise(helper_failure)
      end

      it "removes the git credentials store" do
        expect { with_git_configured { nil } }
          .to raise_error(Dependabot::SharedHelpers::HelperSubprocessFailed, "git config failed")
