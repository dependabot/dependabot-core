  ...
  describe "#create_default_labels_if_required" do
    ...
    context "with GitHub details" do
      ...
      context "when there is a custom dependencies label" do
        ...
        context "when that is only present after paginating" do
        ...
        context "when that should be ignored" do
      ...
      context "when a custom dependencies label has been requested" do
        ...
        context "when that doesn't exist" do
      ...
      context "with an update that fixes a security vulnerability" do
  ...
  describe "#labels_for_pr" do
    ...
    context "with GitHub details" do
      ...
      context "when a 'ruby' label exists" do
        ...
        context "with label_language is true" do
          let(:label_language) { true }
      ...
      context "when asking for custom labels" do
        ...
        context "when that don't exist" do
          let(:custom_labels) { ["non-existent"] }
      ...
      context "with a repo without patch, minor and major labels" do
        it { is_expected.to_not include("patch") }
      end

      context "with a repo that has patch, minor and major labels" do
        ...
        context "with a version and a previous version" do
          ...
          context "with a patch release with build identifier" do
          ...
          context "with a patch release when both have build identifiers" do
          ...
          context "with a minor release" do
            let(:version) { "1.5.1" }
            it { is_expected.to include("minor") }
          end

          context "with a minor release with build identifier" do
            let(:version) { "1.5.1+1" }
            it { is_expected.to include("minor") }
          end

          context "with a minor release when both have build identifiers" do
            let(:previous_version) { "1.4.0+10" }
            let(:version) { "1.5.1+1" }
            it { is_expected.to include("minor") }
          end

          context "with a major release" do
            let(:version) { "2.5.1" }
            it { is_expected.to include("major") }
          end

          context "with a major release with build identifier" do
            let(:version) { "2.5.1+100" }
            it { is_expected.to include("major") }
          end

          context "with a major release when both have build identifiers" do
            let(:previous_version) { "1.4.0+10" }
            let(:version) { "2.5.1+100" }
            it { is_expected.to include("major") }
          end

          context "with a non-semver release" do
            let(:version) { "random" }
            it { is_expected.to eq(["dependencies"]) }
          end
