...
RSpec.describe Dependabot::PullRequestCreator::Github do
  ...
  describe "#create" do
    ...
    context "when creating the branch fails" do
      ...
      context "when because the branch is a superstring of another branch" do
        before do
    ...
    context "when the branch already exists" do
      ...
      context "when but a PR to this branch doesn't" do
        before do
      ...
      context "with a PR to this branch already exists" do
        before do
          url = "#{repo_api_url}/pulls?head=gocardless:#{branch_name}" \
                "&state=all"
          stub_request(:get, url)
        ...
        context "when but isn't initially returned (a race)" do
          before do
            url = "#{repo_api_url}/pulls?head=gocardless:#{branch_name}" \
                  "&state=all"
            stub_request(:get, url)
        ...
        context "when but is merged" do
          ...
          context "when `require_up_to_date_base` is true" do
            ...
            context "with the commit we're branching off of is up-to-date" do
    ...
    context "when a reviewer has been requested" do
      ...
      context "when that can't be added" do
        before do
          stub_request(:post, "#{repo_api_url}/pulls/1347/requested_reviewers")
            .to_return(status: 422,
    ...
    context "when a milestone has been requested" do
      ...
      context "when but can't be specified for some reason" do
        before do
          stub_request(:patch, "#{repo_api_url}/issues/1347")
            .to_return(status: 422,
