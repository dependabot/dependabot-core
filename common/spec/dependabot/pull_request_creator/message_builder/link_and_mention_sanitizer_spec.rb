...
RSpec.describe Dependabot::PullRequestCreator::MessageBuilder::LinkAndMentionSanitizer do
  ...
  describe "#sanitize_links_and_mentions" do
    ...
    context "with an @-mention" do
      ...
      context "when that includes a dash" do
      ...
      context "when that is in brackets" do
      ...
      context "when that is in square brackets" do
        let(:text) { "[@hmarr]" }
      ...
      context "when that appears in single tick code quotes" do
        let(:text) { "Great work `@greysteil`!" }
        it { is_expected.to eq("<p>Great work <code>@greysteil</code>!</p>\n") }
      end

      context "when that appears in double tick code quotes" do
        let(:text) { "Great work ``@greysteil``!" }
        it { is_expected.to eq("<p>Great work <code>@greysteil</code>!</p>\n") }
      end
      ...
      context "when that appears in codeblock quotes" do
        let(:text) { "``` @model ||= 123```" }
        it do
          is_expected.to eq("<p><code> @model ||= 123</code></p>\n")
        end

        context "when that use `~`" do
          let(:text) { "~~~\n @model ||= 123\n~~~" }
          it do
        ...
        context "with mentions inside a complex code fence" do
          ...
          context "with a real mention after" do
            let(:text) do
              "Take a look at this code: ```` @not-a-mention " \
      ...
      context "when multiple team mentions" do
      ...
      context "when team mention and non-mention line" do
