...
| `.required_files_message`        | Returns an error message which can be displayed to a user if `required_files_in?` returns false. Can be optionally passed a directory to customize the message depending on where the fetcher looks for dependency files. |
