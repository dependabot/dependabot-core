...
module Dependabot
  module SharedHelpers # rubocop:disable Metrics/ModuleLength
    ...
    def self.with_git_configured(credentials:, &_block)
      ...
      git_store_dir = Dir.mktmpdir("dependabot_git_store")
      git_store_path = File.join(git_store_dir, "#{random_suffix}.git.store")
      ...
      FileUtils.rm_rf(T.must(git_store_dir))
    end
    ...
    def self.configure_git_to_use_https_with_credentials(
      ...
      github_credentials = github_credentials(credentials)
      github_credential = preferred_github_credential(github_credentials)
      deduped_credentials = credentials -
                            github_credentials +
                            [github_credential].compact

      git_store_content = ""
      hosts = T.let([], T::Array[String])
      deduped_credentials.each do |cred|
        ...
        hosts << cred.fetch("host")
      end
      write_git_store(git_store_path, git_store_content)

      # Then add a file-based credential store that loads a file from the temp dir.
    ...
    sig { params(git_store_path: String, git_store_content: String).void }
    def self.write_git_store(git_store_path, git_store_content)
      File.open(git_store_path, File::WRONLY | File::CREAT | File::TRUNC, 0o600) do |file|
        file.write(git_store_content)
      end
      File.chmod(0o600, git_store_path)
    end

    sig { params(credentials: T::Array[T.untyped]).returns(T::Array[T.untyped]) }
    def self.github_credentials(credentials)
      credentials
    ...
    sig { params(github_credentials: T::Array[T.untyped]).returns(T.untyped) }
    def self.preferred_github_credential(github_credentials)
      ...
      github_credentials.find { |c| !c["password"]&.start_with?("v1.") } || github_credentials.first
    end
