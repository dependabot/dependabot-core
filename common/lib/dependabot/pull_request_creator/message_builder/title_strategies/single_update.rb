    ...
    class MessageBuilder
      module TitleStrategies
        ...
        class SingleUpdate < Base
          ...
          def base_title
            if @library
              "update #{@dependency.display_name} requirement from " \
                "#{@dependency.previous_version} to #{@dependency.version}"
            else
              "bump #{@dependency.display_name} from " \
                "#{@dependency.humanized_previous_version} to #{@dependency.humanized_version}"
            end + directory_suffix
          end
