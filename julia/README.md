...
Reduced to the lowest version per caret line, this becomes the compat entry: `Statistics = "1.10"` for `julia = "1.10"`, `Statistics = "< 0.0.1, 1"` for `julia = "1"`, `SHA = "0.7, 1"` for `julia = "1.10"`. A missing entry is added, and an existing entry that fails to admit one of those versions is widened, regardless of the configured update strategy; entries are never narrowed and no manifest updates are proposed for stdlibs. The pull request carries a notice for each such entry spelling out which Julia releases require which versions and why, since the diff alone does not explain a bound like `< 0.0.1`. Which packages are stdlibs in which release, and at what version, comes from [HistoricalStdlibVersions.jl](https://github.com/JuliaPackaging/HistoricalStdlibVersions.jl), the same data Pkg uses to resolve for a `julia_version` other than the running one. For example, `Artifacts` is only a registry package when the `julia` compat is capped below 1.6, and `StyledStrings` when it is capped below 1.11.
