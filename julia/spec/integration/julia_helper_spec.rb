  ...
  describe "real Julia helper calls" do
    context "with Example package" do
      ...
      it "fetches actual latest version" do
        ...
        # If this fails, let's see what the metadata gives us as latest version
        if result.nil?
          metadata = registry_client.fetch_package_metadata(package_name, package_uuid)

          # Skip the test if fetch_latest_version has issues but metadata works
          skip "fetch_latest_version returned nil, but metadata shows latest_version: #{metadata['latest_version']}"
        else
