...
RSpec.describe Dependabot::Python::FileFetcher do
  ...
  describe "#files" do
    ...
    context "with only a requirements.txt" do
      ...
      context "when that includes comments" do
      ...
      context "when that includes --no-binary" do
      ...
      context "with a todo.txt that is actually a requirements file" do
        before do
      ...
      context "with a todo.txt can't be encoded to UTF-8" do
        before do
          stub_request(:get, url + "todo.txt?ref=sha")
            .with(headers: { "Authorization" => "token token" })
            .to_return(
    ...
    context "with a cascading requirement" do
      ...
      context "when that is fetchable" do
        before do
        ...
        context "with is for a .in file" do
          before do
            stub_request(:get, url + "requirements.txt?ref=sha")
              .with(headers: { "Authorization" => "token token" })
              .to_return(
        ...
        context "with cascades more than once" do
          before do
            stub_request(:get, url + "no_dot/more_requirements.txt?ref=sha")
              .with(headers: { "Authorization" => "token token" })
              .to_return(
    ...
    context "with a path-based dependency that it's fetchable" do
      ...
      context "with references extras" do
      ...
      context "when but is in a child requirement file" do
        before do
          stub_request(:get, url + "requirements.txt?ref=sha")
            .with(headers: { "Authorization" => "token token" })
            .to_return(
      ...
      context "when but is in a Pipfile" do
        let(:repo_contents) do
          fixture("github", "contents_python_only_pipfile_and_lockfile.json")
        end
        let(:directory) { "/docs" }
    ...
    context "with a git dependency" do
      ...
      context "when that uses a git URL" do
        let(:requirements_contents) do
