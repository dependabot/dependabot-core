...
RSpec.describe Dependabot::Python::FileFetcher do
  ...
  describe "#files" do
    ...
    def home_assistant_component_listing_response(path)
      [
        {
          "name" => "manifest.json",
          "path" => File.join(path, "manifest.json"),
          "sha" => "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
          "size" => fixture("home_assistant", "manifest.json").bytesize,
          "url" => "https://api.github.com/repos/gocardless/bump/contents/#{path}/manifest.json?ref=sha",
          "html_url" => "https://github.com/gocardless/bump/blob/sha/#{path}/manifest.json",
          "git_url" => "https://api.github.com/repos/gocardless/bump/git/blobs/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
          "download_url" => "https://raw.githubusercontent.com/gocardless/bump/sha/#{path}/manifest.json",
          "type" => "file",
          "_links" => {
            "self" => "https://api.github.com/repos/gocardless/bump/contents/#{path}/manifest.json?ref=sha",
            "git" => "https://api.github.com/repos/gocardless/bump/git/blobs/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            "html" => "https://github.com/gocardless/bump/blob/sha/#{path}/manifest.json"
          }
        }
      ].to_json
    end
    ...
    before do
      ...
      %w(custom_components homeassistant).each do |dir|
    ...
    context "with Home Assistant custom components" do
      ...
      before do
        ...
        stub_request(:get, url + "custom_components/kia_uvo?ref=sha")
          .with(headers: { "Authorization" => "token token" })
          .to_return(
            status: 200,
            body: home_assistant_component_listing_response("custom_components/kia_uvo"),
            headers: json_header
          )
    ...
    context "with Home Assistant built-in components" do
      ...
      before do
        ...
        stub_request(:get, url + "homeassistant/components/hue?ref=sha")
          .with(headers: { "Authorization" => "token token" })
          .to_return(
            status: 200,
            body: home_assistant_component_listing_response("homeassistant/components/hue"),
            headers: json_header
          )
