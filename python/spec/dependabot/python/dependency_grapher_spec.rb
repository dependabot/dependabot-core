...
RSpec.describe Dependabot::Python::DependencyGrapher do
  ...
  describe "#resolved_dependencies" do
    ...
    context "when a compiled requirements.txt has a dependency with a marker expression" do
      let(:requirements_in) do
        Dependabot::DependencyFile.new(
          name: "requirements.in",
          content: "sqlalchemy>=2.0,<3\n",
          directory: "/"
        )
      end
      ...
      def requirements_txt_with_marker(marker)
        Dependabot::DependencyFile.new(
          name: "requirements.txt",
          content: <<~TXT,
            # This file is autogenerated by pip-compile
      ...
      context "with a satisfied python_version" do
        let(:requirements_txt) do
          requirements_txt_with_marker('python_version >= "3.9"')
        end

        before do
          # Satisfies "python_version >= 3.9".
          allow(parser).to receive(:python_raw_version).and_return("3.11")
        end
      ...
      context "with an unsatisfied python_version" do
        let(:requirements_txt) do
          requirements_txt_with_marker('python_version >= "3.9"')
        end

        before do
          # Does not satisfy "python_version >= 3.9".
          allow(parser).to receive(:python_raw_version).and_return("3.8")
        end
      ...
      context "when only a non-numeric marker is present" do
        let(:requirements_txt) do
          requirements_txt_with_marker('platform_machine == "x86_64"')
        end
      ...
      context "when python_version is joined to a non-numeric marker using 'and'" do
        let(:requirements_txt) do
          requirements_txt_with_marker('python_version >= "3.9" and platform_machine == "x86_64"')
        end
        ...
        it "resolves the dependency's version, since python_version and platform are both satisfied" do
          expect(grapher.resolved_dependencies.keys).to include("pkg:pypi/greenlet@3.5.4")
          expect(grapher.resolved_dependencies.keys).to include("pkg:pypi/sqlalchemy@2.0.43")
        end
      end

      context "when python_version is joined to a non-numeric marker using 'or'" do
        let(:requirements_txt) do
          requirements_txt_with_marker('python_version >= "3.9" or platform_machine == "x86_64"')
        end
        ...
        it "resolves the dependency's version, because 'python_version' alone satisfies the 'or'" do
          expect(grapher.resolved_dependencies.keys).to include("pkg:pypi/greenlet@3.5.4")
          expect(grapher.resolved_dependencies.keys).to include("pkg:pypi/sqlalchemy@2.0.43")
        end
      end

      context "when an unsatisfied python_version is joined to a non-numeric marker using 'and'" do
        let(:requirements_txt) do
          requirements_txt_with_marker('python_version >= "3.9" and platform_machine == "x86_64"')
        end
