...
RSpec.describe Dependabot::Python::FileUpdater::PipCompileFileUpdater do
  ...
  describe "#post_process_compiled_file" do
    subject(:processed_content) do
      updater.send(:post_process_compiled_file, compiled_content, generated_file)
    end

    context "when the compiled file emits a credential-bearing index URL" do
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.txt",
          content: <<~TXT
      ...
      let(:compiled_content) do
        <<~TXT
          # This file is autogenerated by pip-compile
          --index-url https://user:secret-token@pypi.example.com/simple
      ...
      it "strips the credentials from the emitted index URL" do
        expect(processed_content)
          .to include("--index-url https://pypi.example.com/simple")
        expect(processed_content).not_to include("secret-token")
      end
    end

    context "when the compiled file emits a credential-bearing extra index URL" do
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.txt",
          content: <<~TXT
      ...
      let(:compiled_content) do
        <<~TXT
          # This file is autogenerated by pip-compile
          --extra-index-url https://user:secret-token@pypi.example.com/simple
      ...
      it "strips the credentials from the emitted extra index URL" do
        expect(processed_content)
          .to include("--extra-index-url https://pypi.example.com/simple")
        expect(processed_content).not_to include("secret-token")
      end
    end

    context "when the compiled file emits an index URL without credentials" do
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.txt",
          content: <<~TXT
            # This file is autogenerated by pip-compile
            --index-url https://pypi.example.com/simple/@odd-path
      ...
      it "leaves the URL untouched" do
        expect(processed_content)
          .to include("--index-url https://pypi.example.com/simple/@odd-path")
      end
    end

    context "when the token contains an unescaped @ (no colon)" do
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.txt",
          content: <<~TXT
      ...
      let(:compiled_content) do
        <<~TXT
          # This file is autogenerated by pip-compile
          --index-url https://foo@bar@pypi.example.com/simple
      ...
      it "strips the full userinfo through the last @ before the path" do
        expect(processed_content)
          .to include("--index-url https://pypi.example.com/simple")
        expect(processed_content).not_to include("foo@bar")
        expect(processed_content).not_to include("bar@pypi")
      end
    end
  end
