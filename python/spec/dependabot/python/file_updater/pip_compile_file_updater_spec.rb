...
RSpec.describe Dependabot::Python::FileUpdater::PipCompileFileUpdater do
  ...
  describe "#post_process_compiled_file" do
    context "when via annotations have shortened -r paths for subdirectory files" do
      let(:manifest_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.in",
          content: "attrs\nflaky\n"
      ...
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "requirements/test.txt",
          content: original_compiled_content
        )
      end

      let(:original_compiled_content) do
        <<~TXT
          #
      ...
      let(:updated_compiled_content) do
        <<~TXT
          #
          # This file is autogenerated by pip-compile
          #
      ...
      it "restores the full path in via annotations" do
        result = updater.send(:post_process_compiled_file, updated_compiled_content, generated_file)
        expect(result).to include("# via -r requirements/test.in")
        expect(result).not_to include("# via -r test.in")
        expect(result).to include("#   -r requirements/test.in")
        expect(result).not_to include("#   -r test.in")
      end
    end

    context "when via annotations have correct paths (root-level files)" do
      let(:manifest_file) do
        Dependabot::DependencyFile.new(
          name: "test.in",
          content: "attrs\n"
      ...
      let(:generated_file) do
        Dependabot::DependencyFile.new(
          name: "test.txt",
          content: original_root_content
        )
      end

      let(:original_root_content) do
        <<~TXT
          #
          # This file is autogenerated by pip-compile
          #
      ...
      it "leaves the paths unchanged" do
        result = updater.send(:post_process_compiled_file, updated_root_content, generated_file)
        expect(result).to include("# via -r test.in")
      end
    end
  end
end
