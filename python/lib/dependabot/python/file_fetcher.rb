  ...
  module Python
    class FileFetcher < Dependabot::Python::SharedFileFetcher
      ...
      def fetch_home_assistant_manifest_files
        # Home Assistant manifests only live in the two known integration roots, so the
        # scan stays bounded to those directories instead of traversing the entire repo.
        top_level_dirs
        [
      ...
      def fetch_home_assistant_manifests_in(base_dir)
        top_level_dirs = T.must(@top_level_dirs)
        return [] unless top_level_dirs.include?(T.must(base_dir.split("/").first))
