...
module Dependabot
  module Python
    class Requirement < Dependabot::Requirement
      ...
      OR_SEPARATOR = T.let(/(?<=[a-zA-Z0-9)*])\s*\|+/, Regexp) # Add equality and arbitrary-equality matchers
      OPS = T.let(OPS.merge(
                    "==" => ->(v, r) { v == r },
                    "===" => ->(v, r) { v.to_s == r.to_s }
      ...
      def initialize(*requirements)
        ...
        super(requirements)
        @requirements = T.let(@requirements, T::Array[T::Array[T.untyped]])
      end
      ...
      def exact?
        ...
        %w(= == ===).include?(T.must(@requirements[0])[0])
      end

      private

      sig { params(req_string: String).returns(T.any(String, T.nilable(T::Array[T.untyped]))) }
      ...
      def convert_wildcard(req_string)
        ...
        quoted_ops = OPS.keys.sort_by(&:length).reverse
                        .map { |k| Regexp.quote(k) }.join("|")
        op = T.must(req_string.match(/\A\s*(#{quoted_ops})?/))
              .captures.first.to_s.strip
        exact_op = ["", "=", "==", "==="].include?(op)

        star_index = req_string.split(".").index { |s| s.include?("*") }
        req_string.strip
                  .split(".")
                  .first(T.must(star_index) + 1)
                  .join(".")
                  .gsub(/\*(?!$)/, "0")
                  .gsub(/\*$/, "0.dev")
