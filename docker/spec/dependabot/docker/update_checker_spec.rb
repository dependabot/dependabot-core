...
RSpec.describe Dependabot::Docker::UpdateChecker do
  ...
  describe "#up_to_date?" do
    ...
    context "when an out of date digest and an up to date tag" do
      let(:version) { "17.10" }
  ...
  describe "#can_update?" do
    ...
    context "when a purely numeric version" do
      let(:version) { "1234567890" }
      it { is_expected.to be_truthy }
    end

    context "when a non-numeric version" do
      let(:version) { "artful" }
      it { is_expected.to be_falsey }
      ...
      context "with a digest" do
        ...
        context "when that is out-of-date" do
          let(:source) { { digest: "old_digest" } }
          it { is_expected.to be_truthy }

          context "when but the response doesn't include a new digest" do
            let(:headers_response) do
              fixture(
                "docker",
    ...
    context "when a digest only" do
      ...
      context "when that is out-to-date" do
  ...
  describe "#latest_version" do
    ...
    context "when the dependency has a non-numeric version" do
      ...
      context "when that starts with a number" do
    ...
    context "with windows-servercore image" do
      ...
      context "when using versions with cumulative updates (KB)" do
        ...
        context "with we're using a version with no KB" do
          let(:version) { "1803" }
          it { is_expected.to eq("1903-KB4505057") }
        end

        context "with we're using a version with KB" do
    ...
    context "when the dependency has SHA suffices that should be ignored" do
      ...
      context "with an older version of the prefix" do
        let(:version) { "7.1-0.1" }
        it { is_expected.to eq("7.2-0.3") }
      end
    end
    ...
    context "when the dependencies have an underscore" do
      ...
      context "with followed by numbers and with less components than other version but higher underscore part" do
        before do
    ...
    context "when the dependency has a namespace" do
      ...
      context "with dockerhub 401s" do
        before do
          tags_url = "https://registry.hub.docker.com/v2/moj/ruby/tags/list"
          stub_request(:get, tags_url)
            .and_return(
    ...
    context "when the latest version is a pre-release" do
      ...
      context "with the current version is a pre-release" do
        let(:version) { "3.7.0a1" }
    ...
    context "when the dependency has a private registry" do
      ...
      context "with authentication credentials" do
        ...
        context "when that don't have a username or password" do
          let(:credentials) do
            [Dependabot::Credential.new({
              "type" => "git_source",
              "host" => "github.com",
