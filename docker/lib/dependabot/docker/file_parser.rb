...
module Dependabot
  module Docker
    class FileParser < Dependabot::FileParsers::Base
      ...
      ARG = /ARG/i.freeze
      ARG_KEY = /(?<argkey>[a-zA-Z_]{1,}[a-zA-Z0-9_]{0,})/.freeze
      ARG_VAL = /(?<argval>.+)/.freeze
      ARG_LINE = /^#{ARG}\s+#{ARG_KEY}=#{ARG_VAL}$/.freeze
      ...
      def parse
        ...
        dockerfiles.each do |dockerfile|
          dockerfile.content.each_line do |line|
            if FROM.match?(line)
              global_ns = false
              global_args.each { |k, v| line = line.gsub(/\${?#{k}}?/, v) }
            ...
            elsif ARG_LINE.match?(line) && global_ns
              parsed_from_line = ARG_LINE.match(line).named_captures
              global_args.store(parsed_from_line["argkey"],
                                parsed_from_line["argval"])
            end
          end
        end
