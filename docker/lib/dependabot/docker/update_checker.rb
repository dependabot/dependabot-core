...
module Dependabot
  module Docker
    ...
    class UpdateChecker < Dependabot::UpdateCheckers::Base
      ...
      # for single-platform images or when the manifest can't be fetched.
      sig { params(reference: String).returns(T.nilable(T::Hash[String, String])) }
      def platform_digests(reference)
        return platform_digests_cache[reference] if platform_digests_cache.key?(reference)

        digests = fetch_platform_digests_from_registry(reference)
        platform_digests_cache[reference] = digests
        digests
      rescue *transient_docker_errors, DockerRegistry2::RegistryAuthenticationException,
             RestClient::Forbidden, JSON::ParserError => e
        Dependabot.logger.info(
          "Failed to fetch platform digests for #{docker_repo_name}@#{reference}: #{e.message}"
        )
        platform_digests_cache[reference] = nil
        nil
      end

      sig { params(reference: String).returns(T.nilable(T::Hash[String, String])) }
      def fetch_platform_digests_from_registry(reference)
        manifest = with_retries(max_attempts: 3, errors: transient_docker_errors) do
          docker_registry_client.manifest(docker_repo_name, reference)
        end
        ...
        manifests.each_with_object({}) do |m, result|
          ...
          result[platform_key(platform)] = digest
        end
      end
      ...
      sig { returns(T::Hash[String, T.nilable(T::Hash[String, String])]) }
      def platform_digests_cache
        @platform_digests_cache ||= T.let(
          {},
          T.nilable(T::Hash[String, T.nilable(T::Hash[String, String])])
        )
      end
