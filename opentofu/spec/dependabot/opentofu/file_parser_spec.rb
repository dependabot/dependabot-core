...
RSpec.describe Dependabot::Opentofu::FileParser do
  ...
  describe "#parse" do
    ...
    context "when dealing with terraform.lock.hcl files" do
      ...
      it "does not attempt to parse the lockfile" do
        expect { dependencies.length }.to raise_error(RuntimeError) do |error|
          expect(error.message).to eq("No OpenTofu configuration file!")
        end
      end
    end
    ...
    context "with a private module proxy that can't be reached", :vcr do
      before do
        ...
        stub_request(:get, "#{artifactory_repo_url}/terraform-azurerm-nsg-rules.v1.1.0?opentofu-get=1")
  ...
  describe "#source_type" do
    ...
    context "when the source type is a registry" do
      let(:source_string) { "registry.opentofu.org/hashicorp/aws" }
