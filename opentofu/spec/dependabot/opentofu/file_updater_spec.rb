  ...
  describe "#updated_dependency_files" do
    ...
    describe "when updating a provider with mixed case path" do
      ...
      it "updates the module version" do
        ...
        expect(lockfile.content).to include(
          <<~DEP
            provider "registry.opentofu.org/mongey/confluentcloud" {
              version     = "0.0.11"
          DEP
        )
      end
    ...
    describe "when updating a provider with multiple local path modules" do
      ...
      it "updates the module version across all nested providers" do
        ...
        provider_files = updated_files.select { |file| file.name.end_with?(".tf", ".tofu") }
    ...
    describe "#error_handler" do
      ...
      context "when the error message contains no resolvable releases" do
        let(:error_message) do
          ...
          [31m│[0m [0m
          [31m│[0m [0m[OpenTofu failed to fetch the requested providers for linux_amd64 in order
          [31m│[0m [0mto calculate their checksums: some providers could not be installed:
          [31m│[0m [0m- registry.opentofu.org/firehydrant/firehydrant: no available releases
