    ...
    class FileParser < Dependabot::FileParsers::Base
      ...
      class NubLock
        ...
        def dependencies
          ...
          # DependencySet de-dupes by name and keeps the FIRST-inserted entry's metadata, so
          # specifier-bearing (direct) deps go in before bare ones; otherwise an aliased package
          # inserted after a same-name plain entry would lose its `{ alias: name }` tag.
