  ...
  module Nub
    class FileParser < Dependabot::FileParsers::Base
      class LockfileParser
        ...
        DEFAULT_LOCKFILES = %w(package-lock.json yarn.lock pnpm-lock.yaml nub.lock npm-shrinkwrap.json).freeze

        LockFile = T.type_alias { NubLock }
        ...
        sig { returns(T::Array[DependencyFile]) }
        def nub_locks
          @nub_locks ||= T.let(select_files_by_extension("nub.lock"), T.nilable(T::Array[DependencyFile]))
        end

        sig { returns(T.class_of(Dependabot::Nub::Version)) }
        def version_class
          Nub::Version
        end
      end
