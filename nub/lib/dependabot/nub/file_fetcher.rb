  ...
  module Nub
    class FileFetcher < Dependabot::FileFetchers::Base
      ...
      def ecosystem_versions
        ...
        package_managers["nub"] = nub_version if nub_version
      ...
      def fetch_files
        unless allow_beta_ecosystems?
          raise Dependabot::DependencyFileNotFound.new(
            nil,
            "Nub support is currently in beta. Set ALLOW_BETA_ECOSYSTEMS=true to enable it."
          )
        end
        ...
        fetched_files += nub_files if nub_version
      ...
      def nub_files
        fetched_nub_files = []
        fetched_nub_files << nub_lock if nub_lock
        fetched_nub_files
      end
      ...
      def nub_version
        @nub_version ||= T.let(
          package_manager_helper.setup(NubPackageManager::NAME),
      ...
      def nub_lock
        ...
        @nub_lock ||= T.let(fetch_file_if_present(NubPackageManager::LOCKFILE_NAME), T.nilable(DependencyFile))

        return @nub_lock if @nub_lock || directory == "/"

        @nub_lock = fetch_file_from_parent_directories(NubPackageManager::LOCKFILE_NAME)
      end
      ...
      def npmrc
        ...
        @npmrc ||= T.let(fetch_support_file(NubPackageManager::RC_FILENAME), T.nilable(DependencyFile))

        return @npmrc if @npmrc || directory == "/"

        @npmrc = fetch_file_from_parent_directories(NubPackageManager::RC_FILENAME)
      end
...
Dependabot::FileFetchers
  .register(Dependabot::Nub::ECOSYSTEM, Dependabot::Nub::FileFetcher)
