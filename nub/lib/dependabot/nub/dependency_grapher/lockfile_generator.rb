    ...
    class DependencyGrapher < Dependabot::DependencyGraphers::Base
      class LockfileGenerator
        ...
        def run_lockfile_generation
          Dependabot.logger.info("Generating nub.lock for dependency graphing")
          Helpers.run_nub_command("install --ignore-scripts", fingerprint: "install --ignore-scripts")
        end

        sig { returns(Dependabot::DependencyFile) }
        def read_generated_lockfile
          lockfile_name = NubPackageManager::LOCKFILE_NAME
        ...
        def handle_generation_error(error)
          Dependabot.logger.error("Failed to generate nub.lock: #{error.message}")
        end
      end
    end
  end
