...
require "dependabot/nub/file_parser/nub_lock"
...
module Dependabot
  module Nub
    class DependencyGrapher < Dependabot::DependencyGraphers::Base
      ...
      def prepare!
        if lockfile.nil?
          Dependabot.logger.info("No nub.lock found, generating ephemeral lockfile for dependency graphing")
      ...
      def generate_ephemeral_lockfile!
        ...
        Dependabot.logger.info("Successfully generated ephemeral nub.lock for dependency graphing")
      rescue StandardError => e
        errored_fetching_subdependencies!
        @subdependency_error = e
        Dependabot.logger.warn(
          "Failed to generate ephemeral nub.lock: #{e.message}. " \
      ...
      def lockfile
        ...
        @lockfile = T.let(
          dependency_files.find { |f| f.name.end_with?(NubPackageManager::LOCKFILE_NAME) },
          T.nilable(Dependabot::DependencyFile)
        )
      end
      ...
      def fetch_package_relationships
        ...
        parsed_lockfile = FileParser::NubLock.new(T.must(lockfile)).parsed
        packages = parsed_lockfile.fetch("packages", nil)
        return {} unless packages.is_a?(Hash)

        # nub.lock entries are arrays: ["{name}@{version}", registry, {details}, integrity]
...
Dependabot::DependencyGraphers.register("nub", Dependabot::Nub::DependencyGrapher)
