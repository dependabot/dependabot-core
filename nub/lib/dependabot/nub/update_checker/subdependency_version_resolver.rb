...
require "dependabot/nub/update_checker/dependency_files_builder"
    ...
    class UpdateChecker
      class SubdependencyVersionResolver
        ...
        def update_subdependency_in_lockfile(lockfile)
          ...
          updated_files = if lockfile.name.end_with?("nub.lock")
                            run_nub_updater(path, lockfile_name)
                          else
        ...
        def version_from_updated_lockfiles(updated_lockfiles)
          ...
          updated_version = Nub::FileParser.new(
        ...
        def run_nub_updater(path, lockfile_name)
        ...
        sig { returns(Dependabot::Nub::UpdateChecker::DependencyFilesBuilder) }
        def dependency_files_builder
          @dependency_files_builder ||= T.let(
            ...
            T.nilable(Dependabot::Nub::UpdateChecker::DependencyFilesBuilder)
          )
        end
