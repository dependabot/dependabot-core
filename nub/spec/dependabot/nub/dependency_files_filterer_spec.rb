...
RSpec.describe Dependabot::Nub::DependencyFilesFilterer do
  subject(:filterer) do
  ...
  let(:root_lockfile) { Dependabot::DependencyFile.new(name: "nub.lock", content: "{}") }
  ...
  before do
    lockfile_parser = instance_double(Dependabot::Nub::FileParser::LockfileParser, parse: [dependency])
    allow(Dependabot::Nub::FileParser::LockfileParser).to receive(:new).and_return(lockfile_parser)
  end
  ...
  context "with a nested lockfile and no root manifest" do
    let(:root_lockfile) { Dependabot::DependencyFile.new(name: "packages/member/nub.lock", content: "{}") }
