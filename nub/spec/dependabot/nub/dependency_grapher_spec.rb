...
RSpec.describe Dependabot::Nub::DependencyGrapher do
  subject(:grapher) do
  ...
  describe "#relevant_dependency_file" do
    context "when a lockfile exists" do
      let(:dependency_files) { project_dependency_files("nub/grapher_with_lockfile") }

      it "returns the nub.lock file" do
        expect(grapher.relevant_dependency_file.name).to eq("nub.lock")
      end
    end
    ...
    context "when no lockfile exists" do
      ...
      context "when lockfile generation succeeds" do
        before do
          lockfile_generator = instance_double(
            Dependabot::Nub::DependencyGrapher::LockfileGenerator,
            generate: Dependabot::DependencyFile.new(
              name: "nub.lock",
              content: fixture("projects", "nub", "grapher_with_lockfile", "nub.lock"),
              directory: "/"
            )
          )
          allow(Dependabot::Nub::DependencyGrapher::LockfileGenerator)
      ...
      context "when lockfile generation fails" do
        ...
        it "preserves the original error as the subdependency error" do
          ...
          expect(grapher.subdependency_error.message).to include("nub install failed")
        end
  ...
  describe "#resolved_dependencies" do
    ...
    context "with a lockfile containing subdependencies" do
      let(:dependency_files) { project_dependency_files("nub/grapher_with_subdeps") }
    ...
    context "when the lockfile is corrupt" do
      ...
      before do
        ...
        corrupt_lockfile = Dependabot::DependencyFile.new(
          name: "nub.lock", content: "not valid {{{", directory: "/"
        )
  ...
  describe "registration" do
    it "registers as the grapher for the nub package manager" do
      expect(Dependabot::DependencyGraphers.for_package_manager("nub")).to eq(described_class)
    end
  end
end
