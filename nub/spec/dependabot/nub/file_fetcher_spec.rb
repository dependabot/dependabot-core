...
RSpec.describe Dependabot::Nub::FileFetcher do
  ...
  context "with a nub.lock but no package-lock.json file" do
    before do
      stub_request(:get, url + "?ref=sha")
        .with(headers: { "Authorization" => "token token" })
        .to_return(
          status: 200,
          body: fixture("github", "contents_js_nub.json"),
          headers: json_header
        )
      stub_request(:get, File.join(url, "package-lock.json?ref=sha"))
        .with(headers: { "Authorization" => "token token" })
        .to_return(status: 404)
      stub_request(:get, File.join(url, "nub.lock?ref=sha"))
        .with(headers: { "Authorization" => "token token" })
        .to_return(
          status: 200,
          body: fixture("github", "nub_lock_content.json"),
          headers: json_header
        )
    end

    describe "fetching and parsing the nub.lock" do
      before do
        allow(Dependabot::Experiments).to receive(:enabled?)
      end

      it "fetches and parses the nub.lock" do
        expect(file_fetcher_instance.files.map(&:name))
          .to match_array(%w(package.json nub.lock))
        expect(file_fetcher_instance.ecosystem_versions)
          .to match({ package_managers: { "nub" => an_instance_of(Integer) } })
      end
    end
  end
