...
module Dependabot
  module Maven
    module Shared
      class SharedMetadataFinder < Dependabot::MetadataFinders::Base
        extend T::Sig
        ...
        def repo_has_subdir_for_dep?(tmp_source)
          @repo_has_subdir_for_dep = T.let(
            @repo_has_subdir_for_dep, T.nilable(T::Hash[Dependabot::Source, T::Boolean])
          )
          @repo_has_subdir_for_dep ||= {}
          return T.must(@repo_has_subdir_for_dep[tmp_source]) if @repo_has_subdir_for_dep.key?(tmp_source)

          fetcher = file_fetcher_class.new(source: tmp_source, credentials: credentials)
        ...
        sig { overridable.returns(T.class_of(Dependabot::FileFetchers::Base)) }
        ...
        sig { overridable.returns(Regexp) }
        def property_regex
        ...
        def dependency_pom_file
          @dependency_pom_file = T.let(@dependency_pom_file, T.nilable(Nokogiri::XML::Document))
          return @dependency_pom_file if @dependency_pom_file

          response = Dependabot::RegistryClient.get(
            url: "#{maven_repo_dependency_url}/" \
                 "#{dependency.version}/#{dependency_artifact_id}-#{dependency.version}.pom",
        ...
        def auth_headers
          @auth_headers ||= T.let(
            Dependabot::Maven::Utils::AuthHeadersFinder
                                      .new(credentials)
                                      .auth_headers(maven_repo_url),
