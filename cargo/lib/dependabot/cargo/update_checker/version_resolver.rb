    ...
    class UpdateChecker
      ...
      class VersionResolver
        ...
        def initialize(dependency:, credentials:,
          ...
          @unreachable_git_urls = T.let(nil, T.nilable(T::Array[String]))
          @reachable_git_urls = T.let(nil, T.nilable(T::Array[String]))
          @prepared_manifest_files = T.let(nil, T.nilable(T::Array[Dependabot::DependencyFile]))
          @original_manifest_files = T.let(nil, T.nilable(T::Array[Dependabot::DependencyFile]))
        ...
        def better_specification_needed?(error)
          ...
          ver_string = T.must(ver)
          if spec_options.count { |s| s.end_with?(ver_string) } == 1
            @custom_specification = spec_options.find { |s| s.end_with?(ver_string) }
            return true
          elsif spec_options.count { |s| s.end_with?(ver_string) } > 1
            spec_options.select! { |s| s.end_with?(ver_string) }
          end

          if git_dependency? && git_source_url
            git_url = T.must(git_source_url)
            spec_options.select! { |s| s.include?(git_url) } if spec_options.count { |s| s.include?(git_url) } >= 1
          end
        ...
        def write_temporary_dependency_files(prepared: true)
          ...
          if lockfile
            lockfile_obj = T.must(lockfile)
            File.write(lockfile_obj.name, lockfile_obj.content)
          end
          if toolchain
            toolchain_obj = T.must(toolchain)
            File.write(toolchain_obj.name, toolchain_obj.content)
          end
          return unless config

          config_obj = T.must(config)
          FileUtils.mkdir_p(File.dirname(config_obj.name))
          File.write(config_obj.name, config_obj.content)
        end
        ...
        def handle_cargo_errors(error)
          ...
          if error.message.include?("authenticate when downloading repo") ||
            ...
            if urls.none?
              match = error.message.match(UNABLE_TO_UPDATE)
              if match
                url_raw = match.named_captures["url"]
                if url_raw
                  url = url_raw.split(/[#?]/).first
                  if url
          ...
          [BRANCH_NOT_FOUND_REGEX, REF_NOT_FOUND_REGEX, GIT_REF_NOT_FOUND_REGEX, NOT_OUR_REF_REGEX].each do |regex|
            ...
            dep_match = error.message.match(regex)
            next unless dep_match

            dep_url_raw = dep_match.named_captures["url"]
            next unless dep_url_raw

            dependency_url = dep_url_raw.split(/[#?]/).first
        ...
        def unreachable_git_urls
          return T.must(@unreachable_git_urls) if defined?(@unreachable_git_urls)
        ...
        def reachable_git_urls
          return T.must(@reachable_git_urls) if defined?(@reachable_git_urls)

          unreachable_git_urls
          T.must(@reachable_git_urls)
        end
        ...
        def git_dependency_version
          ...
          lockfile_obj = T.must(lockfile)
          TomlRB.parse(lockfile_obj.content)
                .fetch("package", [])
                .select { |p| p["name"] == dependency.name }
