...
RSpec.describe Dependabot::Cargo::FileFetcher do
  ...
  context "with a path dependency" do
    ...
    context "when that is fetchable" do
      ...
      context "with a target dependency" do
        let(:parent_fixture) do
          fixture(
            "github",
      ...
      context "with a replacement source" do
        let(:parent_fixture) do
      ...
      context "with a patched source" do
        let(:parent_fixture) do
      ...
      context "with includes another path dependency" do
        let(:path_dep_fixture) do
          fixture("github", "contents_cargo_manifest_path_deps.json")
        end
    ...
    context "when that is not fetchable" do
      before do
        stub_request(:get, url + "src/s3/Cargo.toml?ref=sha")
          .with(headers: { "Authorization" => "token token" })
          .to_return(status: 404, headers: json_header)
  ...
  context "with a workspace dependency" do
    ...
    context "when that is fetchable" do
      ...
      context "with specifies the dependency implicitly" do
        let(:parent_fixture) do
      ...
      context "with specifies the dependency as a path dependency, too" do
        let(:parent_fixture) do
          fixture(
            "github",
    ...
    context "when that is in a submodule" do
      before do
    ...
    context "when that specifies a directory of packages" do
      let(:parent_fixture) do
