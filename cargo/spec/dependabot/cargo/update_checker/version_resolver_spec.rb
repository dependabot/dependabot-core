...
RSpec.describe Dependabot::Cargo::UpdateChecker::VersionResolver do
  ...
  describe "latest_resolvable_version" do
    ...
    context "with a dependency that doesn't exist" do
      ...
      context "when which isn't the package being updated" do
        let(:dependency_name) { "regex" }
        let(:string_req) { "0.1.41" }
    ...
    context "with a git dependency" do
      ...
      context "when that is unreachable" do
        ...
        context "when but is skipped by the parser (because it has multiple URLs)" do
    ...
    context "when there is a workspace" do
      ...
      context "when but Dependabot has been asked to run on only a child" do
      ...
      context "when but it is not correctly set up" do
        let(:unprepared_dependency_files) do
