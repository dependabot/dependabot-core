...
RSpec.describe Dependabot::PreCommit::FileParser do
  ...
  describe "#parse" do
    ...
    context "with a prek.toml (TOML) config" do
      ...
      it "parses a remote repo into a pre_commit git dependency" do
        dep = dependencies.find { |d| d.name.include?("pre-commit-hooks") }
        expect(dep).not_to be_nil
        expect(dep.name).to eq("https://github.com/pre-commit/pre-commit-hooks")
        expect(dep.version).to eq("v4.4.0")
        expect(dep.package_manager).to eq("pre_commit")
        expect(dep.requirements).to eq(
          [{
            requirement: nil,
            groups: [],
      ...
      context "when a later table's rev precedes its repo" do
        # The second table puts `rev` before `repo` (legal TOML, any key order)
        # and carries a comment. The first repo's own rev line has no comment,
        # so a leaked current_repo would misattribute the second table's
        # comment to the first repo.
        let(:pre_commit_config) do
          Dependabot::DependencyFile.new(
            name: "prek.toml",
            content: fixture("prek_configs", "rev_before_repo.toml")
          )
        end

        it "does not attribute a later table's comment to an earlier repo" do
          dep = dependencies.find { |d| d.name.include?("pre-commit-hooks") }
          expect(dep.requirements.first[:metadata][:comment]).to be_nil
        end
      end
