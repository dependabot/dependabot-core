  ...
  module Terraform
    class UpdateChecker < Dependabot::UpdateCheckers::Base
      ...
      def latest_version_for_registry_dependency
        ...
        latest_version_resolver.filter_versions_in_cooldown_period_from_module(versions) if cooldown_enabled?
      ...
      def latest_version_for_provider_dependency
        ...
        # Filter versions which are in cooldown period
        latest_version_resolver.filter_versions_in_cooldown_period_from_provider(versions) if cooldown_enabled?
        versions.reject!(&:prerelease?) unless wants_prerelease?
        versions.reject! { |v| ignore_requirements.any? { |r| r.satisfied_by?(v) } }
